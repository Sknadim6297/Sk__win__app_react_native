const App = (() => {
  ...
  function formatLabel(row) {
    ...
    if (row.matchType === 'Custom Match' || row.category === 'custom') {
  ...
  function isCustom(row) {
    return row.matchKind === 'team_vs_team' || row.category === 'custom' || row.tournamentType === 'custom_match';
  }
  ...
  function formatOptions(category, selected) {
  ...
  function modeDisplay(detail, t, custom) {
    if (custom) {
      const ppt = detail.playersPerTeam || (t.mode === 'squad' ? 4 : t.mode === 'duo' ? 2 : 1);
      return `${detail.formatLabel || formatLabel(t)} · ${ppt} player${ppt > 1 ? 's' : ''} per team`;
    }
    const label = detail.modeLabel || String(t.mode || 'solo');
          ...
          ${tournaments.length ? tournaments.map((row) => `
            <div class="list-row">
              <div>
                ...
                <div style="color:var(--text-2);font-size:12px">${AdminUI.esc(row.matchType || '')} · ${AdminUI.esc(formatLabel(row))}${row.map ? ` · ${AdminUI.esc(row.map)}` : ''}</div>
              </div>
              ${badge(row.status)}
    ...
    return items.map((row) => {
      ...
      return `
        <tr>
          ...
          <td>${AdminUI.esc(row.matchType || (isCustom(row) ? 'Custom Match' : 'Battle Royale'))}</td>
    ...
    return `<div class="mobile-list">${items.map((row) => {
      ...
      return `
        <div class="m-card">
          ...
          <div class="m-row"><span>Type</span><b>${AdminUI.esc(row.matchType || '—')}</b></div>
          <div class="m-row"><span>Mode</span><b>${AdminUI.esc(formatLabel(row))}</b></div>
    ...
    root().innerHTML = AdminUI.layout(nav, `
      ...
      <div class="card">
        <div class="table-wrap desktop-table">
          <table>
            <thead><tr>
              <th>Tournament</th><th>Match type</th><th>Mode</th><th>Map</th><th>Slots</th>
  ...
  async function tournamentForm(id) {
    ...
    AdminUI.bindShell();
    const [games, maps, existing] = await Promise.all([
      AdminAPI.games(),
      AdminAPI.maps().catch(() => []),
    ...
    root().innerHTML = AdminUI.layout(nav, `
      ...
      <form id="t-form" class="form-card">
        <div class="form-grid">
          ...
          <div class="form-section">
            <h3>Match setup</h3>
            <p>Type, team size, map and schedule. Custom Match is Team A vs Team B. Battle Royale uses a slot grid.</p>
          </div>
          <div class="field"><label>Match type <span class="req">*</span></label>
            <select name="category" id="f-cat">
              <option value="custom" ${t.category === 'custom' || t.category === 'custom_match' ? 'selected' : ''}>Custom Match (1v1 / 2v2 / 4v4)</option>
              <option value="battle_royale" ${t.category === 'battle_royale' || !t.category ? 'selected' : ''}>Battle Royale (Solo / Duo / Squad)</option>
            </select>
          </div>
          <div class="field"><label id="f-format-label">Mode</label>
            <select name="mode" id="f-format">${formatOptions(t.category === 'custom' || t.category === 'custom_match' ? 'custom' : 'battle_royale', t.mode || 'solo')}</select>
            <span class="help" id="f-format-help"></span>
          </div>
          ...
          <div class="form-section">
            <h3>Entry & prizes</h3>
            <p>Entry is charged per team for Custom Match, and per slot/team for Battle Royale. Per-kill is Battle Royale only.</p>
          </div>
          ...
          <div class="field" id="kill-wrap"><label>Per-kill reward</label><input name="perKill" type="number" min="0" value="${t.perKill ?? 0}" /><span class="help">Shown in match details for Battle Royale. Hidden for Custom Match.</span></div>
    ...
    const syncMatchType = () => {
      ...
      document.getElementById('f-format-help').textContent = cat === 'custom'
        ? 'Custom Match is always Team A vs Team B. Choose 1v1, 2v2 or 4v4.'
    ...
    document.getElementById('f-cat').onchange = syncMatchType;
    document.getElementById('back').onclick = () => go('tournaments');
    ...
    document.getElementById('t-form').onsubmit = (e) => {
      ...
      guarded(async () => {
        ...
        body.perKill = body.category === 'custom' ? 0 : Number(body.perKill || 0);
        ...
        if (!body.name || !body.game || !body.gameMode || !body.startDate) {
          AdminUI.toast('Name, game, mode and start time are required', 'err');
          return;
        }
  ...
  async function tournamentDetail(id, tab = 'overview') {
    ...
    const overview = `
      ...
      <div class="info-grid">
        ${infoCell('Match type', AdminUI.esc(detail.matchType || (custom ? 'Custom Match' : 'Battle Royale')))}
      ...
      <div class="panel" style="margin-top:16px">
        ...
        <dl class="kv">
          ...
          <dt>Match type</dt><dd>${AdminUI.esc(detail.matchType || '')}</dd>
      ...
      <div class="detail-head">
        <div>
          ...
          <div class="chip-row">
            ...
            <span class="badge b-primary">${AdminUI.esc(detail.matchType || (custom ? 'Custom Match' : 'Battle Royale'))}</span>
        ...
        <div class="table-wrap desktop-table"><table>
          ...
          <tbody>${items.map((row) => `
            <tr>
              ...
              <td>${AdminUI.esc(row.matchType)}</td>
  ...
  async function historyDetail(id) {
    ...
    root().innerHTML = AdminUI.layout('history', `
      ${AdminUI.pageHead(t.name || 'History detail', `${t.matchType || ''} · booked ${t.bookedSlots}/${t.totalSlots}`, `<button class="btn btn-secondary" id="back">Back to history</button>`)}
    ...
    root().innerHTML = AdminUI.layout(nav, `
      ...
      <div class="card">
        <div class="table-wrap desktop-table"><table>
          ...
          <tbody>${items.map((tx) => `
            <tr>
              <td>${AdminUI.esc(tx.transactionId || tx.cashfreePaymentId || tx._id)}</td>
  ...
  function modeFields(m = {}) {
    return `
      <div class="field"><label>Mode name <span class="req">*</span></label><input name="name" required value="${AdminUI.esc(m.name || '')}" placeholder="Battle Royale / Custom Match" /></div>
        ...
        <div class="panel" style="margin-bottom:14px">
          ...
          <div class="table-wrap" style="margin-top:12px"><table>
            ...
            <tbody>${modes.map((m) => `<tr>
              <td>${m.image ? AdminUI.img(m.image, 'mode-thumb') : ''}${AdminUI.esc(m.name)}</td>
            ...
            </tr>`).join('') || `<tr><td colspan="3">${AdminUI.empty('No modes yet', 'Add Battle Royale and Custom Match modes with images.')}</td></tr>`}</tbody>
          </table></div>
  ...
  async function dailyAutoMatches() {
    ...
    root().innerHTML = AdminUI.layout('daily-auto', `
      ${AdminUI.pageHead(
        'Daily Auto Matches',
        'Master templates. The backend creates a real tournament from each active config every day at 12:05 AM IST. Editing a generated tournament never changes the master.',
      ...
      <div class="card">
        <div class="table-wrap desktop-table">
          <table>
            <thead><tr>
              <th>ID</th><th>Game</th><th>Tournament</th><th>Match time</th>
              <th>Entry fee</th><th>Prize pool</th><th>Status</th><th>Next match</th><th>Actions</th>
            </tr></thead>
            <tbody>${items.length ? items.map((row) => `
              <tr>
                <td><strong>${AdminUI.esc(row.displayId || `AUTO${row.autoMatchNumber}`)}</strong></td>
                <td>${AdminUI.esc(row.game?.name || '—')}</td>
                <td>
                  <a class="name-link" href="#/daily-auto/${row._id}/edit">${AdminUI.esc(row.name)}</a>
                  <span class="auto-source">${AdminUI.esc(row.category === 'custom' || row.category === 'custom_match' ? 'Custom Match' : 'Battle Royale')} · ${AdminUI.esc((row.mode || 'solo').toUpperCase())}</span>
                </td>
              ...
              </tr>`).join('') : `<tr><td colspan="9">${AdminUI.empty('No Daily Auto Matches yet', 'Create a master config once. The system will generate the actual tournament every day.')}</td></tr>`}</tbody>
          </table>
        </div>
        ${items.length ? `<div class="mobile-list">${items.map((row) => `
          <div class="m-card">
            ...
            <div class="m-row"><span>Game</span><b>${AdminUI.esc(row.game?.name || '—')}</b></div>
          ...
          </div>`).join('')}</div>` : `<div class="mobile-list">${AdminUI.empty('No Daily Auto Matches yet')}</div>`}
      </div>`);
    AdminUI.bindShell();
  ...
  async function dailyAutoGenerated(id) {
    ...
    root().innerHTML = AdminUI.layout('daily-auto', `
      ${AdminUI.pageHead(
        ...
        `${auto.displayId || ''} · Each row is a real tournament copied from this master on that date.`,
      ...
      <div class="card">
        <div class="table-wrap desktop-table">
          <table>
            <thead><tr>
              <th>Date</th><th>Tournament</th><th>Match #</th><th>Entry fee</th><th>Prize pool</th><th>Status</th><th></th>
            </tr></thead>
            <tbody>${items.length ? items.map((row) => `
              <tr>
              ...
              </tr>`).join('') : `<tr><td colspan="7">${AdminUI.empty('No generated tournaments yet', 'Use Generate today’s match or wait for the 12:05 AM IST job.')}</td></tr>`}</tbody>
          </table>
        </div>
        ${items.length ? `<div class="mobile-list">${items.map((row) => `
          <div class="m-card">
  ...
  async function dailyAutoForm(id) {
    ...
    root().innerHTML = AdminUI.layout('daily-auto', `
      ...
      <form id="auto-form" class="form-card">
        <div class="form-grid">
          ...
          <div class="form-section">
            <h3>Match setup</h3>
            <p>The date is filled in at generation time. You only set the daily clock time (India).</p>
          </div>
          <div class="field"><label>Match type <span class="req">*</span></label>
            <select name="category" id="f-cat">
              <option value="custom" ${t.category === 'custom' || t.category === 'custom_match' ? 'selected' : ''}>Custom Match (1v1 / 2v2 / 4v4)</option>
    ...
    document.getElementById('auto-form').onsubmit = (e) => {
      ...
      guarded(async () => {
        ...
        body.repeat = 'daily';
        if (!body.name || !body.game || !body.gameMode || !body.startTime) {
          AdminUI.toast('Name, game, mode and start time are required', 'err');
          return;
        }
        if (id) {
  ...
  async function render() {
    ...
    try {
      ...
      if (section === 'teams') return await opsList('teams', 'Teams', 'Custom Match team occupancy. Open a match to manage Team A / Team B.', { category: 'custom' });
