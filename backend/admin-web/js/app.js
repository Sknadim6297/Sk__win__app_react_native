const App = (() => {
  ...
  function displayMatchType(row) {
    if (isCustom(row) || /custom match/i.test(String(row.matchType || ''))) return 'Clash Squad';
    return toPlayerMatchLabel(row.matchType) || 'Battle Royale';
  }
    ...
    root().innerHTML = AdminUI.layout(nav, `
      ...
      <form id="t-form" class="form-card">
        <div class="form-grid">
          ...
          <div class="form-section">
            <h3>Match setup</h3>
            <p>Type, team size, map and schedule. Clash Squad is Team A vs Team B. Battle Royale uses a slot grid.</p>
          </div>
          <div class="field"><label>Match type <span class="req">*</span></label>
            <select name="category" id="f-cat">
              <option value="custom" ${t.category === 'custom' || t.category === 'custom_match' ? 'selected' : ''}>Clash Squad (1v1 / 2v2 / 4v4)</option>
            ...
            <tbody>${items.length ? items.map((row) => `
              <tr>
                ...
                <td>
                  ...
                  <span class="auto-source">${AdminUI.esc(row.category === 'custom' || row.category === 'custom_match' ? 'Clash Squad' : 'Battle Royale')} · ${AdminUI.esc((row.mode || 'solo').toUpperCase())}</span>
                </td>
